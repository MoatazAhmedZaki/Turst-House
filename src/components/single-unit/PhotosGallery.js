...
export default class PhotosGallery extends Component {
  ...
  onThumbnail(e) {
    var index=this.state.currentImage-1
    this.setState({
      currentImage: e})
   
    console.log("Thumbnail photo number is : " + index)  }
...
const width=4;
    const height=4;
const imgs=this.props.unit
if(imgs){
 imgs.map((src )=> photos.push({src,width,height}) );
 ...
 onClick={this.openLightbox}
    ...
    />
        ...
        <Lightbox images={photos}
          ...
          showThumbnails={true}
          onClickThumbnail={this.onThumbnail}
        />
      </div>
    )
  }
